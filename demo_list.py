...
class DemoContent:
    def __init__(self, parent_root, command):
        ...
        self.window = None
        self.examples = self.get_list_examples()
    ...
    def show(self):
        ...
        title_label = tk.Label(main_frame, text="Demo: list()", font=('Arial', 14, 'bold'), bg='white')
        ...
        usage_text.insert('1.0', 'list("abc")\nlist(range(3))\n[1, 2, 3] + [4, 5]\n[0] * 5')
    ...
    def get_list_examples(self):
        return [
            {'title': 'From a string', 'description': 'Convert each character of a string to a list item', 'default_input': 'list("hello")'},
            {'title': 'Using range()', 'description': 'Make a list from 0 to 4 using range()', 'default_input': 'list(range(5))'},
            {'title': 'Manual List', 'description': 'Define a list with specific numbers', 'default_input': '[10, 20, 30]'},
            {'title': 'Repeat Elements', 'description': 'Create a list with five zeros', 'default_input': '[0] * 5'},
            {'title': 'List Concatenation', 'description': 'Join two lists together', 'default_input': '[1, 2] + [3, 4]'},
            {'title': 'List Comprehension', 'description': 'Create a list of squares using a loop', 'default_input': '[x**2 for x in range(5)]'},
            {'title': 'Count Items', 'description': 'Count how many times a value appears', 'default_input': '[1, 2, 2, 3].count(2)'},
            {'title': 'Indexing', 'description': 'Find the position of an item', 'default_input': '[3, 1, 4].index(4)'},
            {'title': 'Sorting', 'description': 'Sort a list in ascending order', 'default_input': 'sorted([4, 2, 1])'},
            {'title': 'Filter Even', 'description': 'Use filter and list to get even numbers', 'default_input': 'list(filter(lambda x: x % 2 == 0, range(10)))'},
        ]
